  ...
  return `
    <div class="order-card">
      ...
      <div class="order-card-actions">
        ...
        ${renderCancelBtn(order, id, preview)}
      </div>
    </div>`;
}
...
/* ─── Cancel Order Button ─────────────────────────────────── */
function renderCancelBtn(order, orderId, itemsPreview) {
  // Only allow cancellation for pending / received orders
  const status = (order.status || '').toLowerCase();
  const cancellable = ['pending', 'received'].includes(status);
  if (!cancellable) return '';

  const msg = `I would like to CANCEL my order #${orderId}.\n\nItems: ${itemsPreview}\nTotal: ${formatCurrency(order.total)}\n\nKindly confirm the cancellation and initiate a refund if applicable.`;

  return `
    <button
      class="order-cancel-btn"
      data-wa-msg="${msg.replace(/"/g, '&quot;')}"
      onclick="window.App.WA.open(this.dataset.waMsg);"
      title="Request order cancellation via WhatsApp">
